...
(my $telegram = join "", <0>) =~ s/.*^\s*use\s+Morse\s*;\n//sm;
...
Morse - Perl programming in morse code
...
The first time you run a program under C<use Morse>, the module converts
